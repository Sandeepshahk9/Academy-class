                ...
                <div class="box">
                    ...
                    <div class="box-body table-responsive" >
        		  		<table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php $q=mysqli_query($con,"select enquiry.*,course.name as course_name,session.name as session_name
                                from enquiry
                                left join course on course.id=enquiry.course_id
                                left join session on session.id=enquiry.session_id
                                order by enquiry.id desc");
                                $i = 1;	
                                while($r=mysqli_fetch_array($q)){ ?>
                                    <tr>
                                        <td>
                                            <a href="./enquiry.php?upid=<?php echo $r["id"]; ?>"><i class="fa fa-pencil text-yellow"></i></a>&nbsp;&nbsp; 
                                            <a href="./enquiry.php?did=<?php echo $r["id"]; ?>" onclick="return confirm_click();"><i class="fa fa-remove text-red"></i></a>
                                        </td>
                                        <td>
                                            <?php
                                                $check=mysqli_num_rows(mysqli_query($con,"select id from register where enquiry_id=".$r['id']));
                                                if(empty($check)){
                                            ?>
                                            <a href="enquiry_register.php?upid=<?=$r['id'];?>"><button class="btn btn-primary">Register</button></a>
                                            <?php
                                                }else{
                                                    echo "<span class='text-success'>Registered</span>";
                                                }
                                            ?>
                                        </td>
                                        <td><?=$i;?></td>
